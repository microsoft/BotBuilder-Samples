...
class LGCompletionItemProvider {
        ...
        if (/\[[^\]]*\]\([^)]*$/.test(lineTextBefore) && !util.isInFencedCodeBlock(document, position)) {
            ...
            if (/>\s!#\s*$/.test(lineTextBefore)) {
                ...
                if (/>\s!#\s*@strict\s*=$/.test(lineTextBefore)) {
                    ...
                    const templatesOptions = templatesStatus_1.TemplatesStatus.templatesMap.get(document.uri.fsPath).templates.toArray();
    ...
    isStartStructure(document, position) {
        const lineTextBefore = document.lineAt(position.line).text.substring(0, position.character);
    ...
    isInStructure(document, position) {
        if (util.isInFencedCodeBlock(document, position)
            || position.line <= 0)
