...
async function compareDirs(original: string, merged: string): Promise<Comparison> {
    ...
    for (let file1 of originalFiles) {
        if (mergedFiles.has(file1)) {
            ...
            let originalVal = await fs.readFile(ppath.join(original, file1))
            let mergedVal = await fs.readFile(ppath.join(merged, file1))

            if(file1.endsWith('sandwichMerge.en-us.lu')){
                console.log(`>>${originalVal.length}<<`)
                console.log(`>>${originalVal.join(' ')}<<`)
                console.log('***')
                console.log(`>>${mergedVal.length}<<`)
                console.log(`>>${mergedVal.join(' ')}<<`)
            } 
