...
export default class GenerateDialog extends Command {
    ...
    static flags: flags.Input<any> = {
        ...
        templates: flags.string({char: 't', description: 'Directory with templates to use for generating assets.  With multiple directories, the first definition found wins.  To include the standard templates, just use "template:standard" as a template directory name.', multiple: true}),
        transforms: flags.string({char: 'T', description: 'Global transforms to include.', multiple: true}),
    ...
    async run() {
        ...
        try {
            await gen.generate(args.schema, {
                prefix: flags.prefix, 
                outDir: flags.output,
                metaSchema: flags.schema, 
                locales: flags.locale, 
                templateDirs: flags.templates, 
                transforms: flags.transforms, 
                force: flags.force, 
                merge: flags.merge, 
                singleton: !flags.files, 
                feedback})
            return true
        } catch (e) {
            this.thrownError(e)
