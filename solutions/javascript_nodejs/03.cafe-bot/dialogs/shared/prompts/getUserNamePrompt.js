        ...
        super(dialogId, async (promptValidatorContext) => {
            ...
            if (userProfile !== undefined && userProfile.userName !== undefined) {
                ...
                if (userProfile.userName.split(' ').length > 2) {
                    await promptValidatorContext.context.sendActivity(`Sorry, I can only accept two words for a name.`);
                    await promptValidatorContext.context.sendActivity(`You can always say 'My name is <your name>' to introduce yourself to me.`);
                    await this.userProfileAccessor.set(promptValidatorContext.context, new UserProfile('Human'));
                    // set updated turn counter
                    await this.turnCounterAccessor.set(promptValidatorContext.context, 0);
                    ...
                    await this.userProfileAccessor.set(promptValidatorContext.context, userProfile);
