Basic bot template that puts together cards, NLP (LUIS) with ASP.Net Web API. 
# To try this sample
...
- Navigate to [LUIS portal](https://www.luis.ai).
- Click the `Sign in` button.
- Click on the `Choose File` and select [greeting.luis](greeting.luis) from the `BotBuilder-Samples\csharp_webapi\13.Basic-Bot-Template\CognitiveModels` folder.
