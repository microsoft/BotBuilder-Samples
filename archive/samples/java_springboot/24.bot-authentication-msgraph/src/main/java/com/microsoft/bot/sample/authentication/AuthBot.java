...
public class AuthBot<T extends Dialog> extends DialogBot {
    ...
    protected CompletableFuture<Void> onMembersAdded(
        ...
        return turnContext.getActivity().getMembersAdded().stream()
            ...
            .map(channel -> {
                Activity reply = MessageFactory.text("Welcome to Authentication Bot on MSGraph."
    ...
    protected CompletableFuture<Void> onTokenResponseEvent(TurnContext turnContext) {
        LoggerFactory.getLogger(AuthBot.class).info("Running dialog with Teams Signin Verify State Activity.");
