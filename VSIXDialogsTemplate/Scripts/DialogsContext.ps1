...
{    
	...
	if ($Line -match $paternBotClass) 
    {
		...
		$FileModified += "			{"
		$FileModified += "				%%Step1%%Async,"
		$FileModified += "				%%Step1%%ConfirmAsync,"
		$FileModified += "				%%Step2%%Async,"
		$FileModified += "				%%Step2%%ConfirmAsync,"
		...
		$FileModified += "			_dialogs.Add(new TextPrompt(""%%Step1%%""));"
		$FileModified += "			_dialogs.Add(new NumberPrompt<int>(""%%Step2%%""));"
