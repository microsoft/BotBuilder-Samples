# event hub transcript logger sample
Bot Framework v4 event hub transcript logger bot sample

This bot has been created using [Microsoft Bot Framework][1], it shows how to create a bot that demonstrates storing transcript logs of the conversation. The file `eventHubTranscriptLogger.js` contains a custom implementation of the logging middleware.

This implementation contains an object that saves ongoing conversations to the configured Azure Event Hubs.  Events are batched and thresholds can be set via the `hub_batch_size` (for the number of events) and `hub_batch_interval_ms`(for a timeout threshold).
...
- Navigate to `samples/javascript_nodejs/27.eventhub-logger` folder
...
[28]: https://azure.microsoft.com/en-us/services/event-hubs/
