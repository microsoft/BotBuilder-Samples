...
class RootBot extends ActivityHandler {
    constructor(conversationState, skillsConfig, skillClient, conversationIdFactory) {
        ...
        this.skillConversationIdProperty = this.conversationState.createProperty(RootBot.SkillConversationIdPropertyName);
        ...
        this.onTurn(async (turnContext, next) => {
            ...
            if (turnContext.activity.type !== ActivityTypes.EndOfConversation) {
                ...
                const skillConversationId = await this.skillConversationIdProperty.get(turnContext);

                if (activeSkill && skillConversationId) {
                    // Send the activity to the skill
                    await this.sendToSkill(turnContext, activeSkill, skillConversationId);
                    return;
                }
            }
        ...
        this.onMessage(async (context, next) => {
            if (context.activity.text.toLowerCase() === 'skill') {
                ...
                // Set conversation id
                await this.skillConversationIdProperty.set(context, skillConversationId);

                // Send the activity to the skill
                await this.sendToSkill(context, this.targetSkill, skillConversationId);
            } else {
        ...
        this.onEndOfConversation(async (context, next) => {
            ...
            await this.skillConversationIdProperty.set(context, undefined);
    ...
    async sendToSkill(context, targetSkill, skillConversationId) {
...
RootBot.SkillConversationIdPropertyName = 'skillConversationIdProperty';
